...
- 本文档聚焦“标准 CRUD 页面”
- 通用弹窗约束请同时遵守 `docs/08-弹窗组件规范.md`
- `FormDialog.vue` 只是 CRUD 页面里的一个特例，并不是所有弹窗都必须叫这个名字
...
- 如果是非 CRUD 的业务弹窗，可改为 `XxxActionDialog.vue`
- 如果是只读详情弹窗，可改为 `XxxDetailDialog.vue`
- 具体命名与职责边界以 `docs/08-弹窗组件规范.md` 为准
...
- 编辑时在 `open` 中回显
- 关闭时在 `closed` 中重置
- 关闭弹窗时优先仅使用 `resetFields()`，对齐当前用户管理弹窗实现
- 仅当存在 `resetFields()` 无法覆盖的额外本地状态时，再补充手动清理逻辑
- 私有弹窗允许直接调用新增/编辑接口
